...
const convertToCSV = (users) => {
  const headers = [
    ...
    "Work End Date",
    "Client Partner Location",
    "Client Manager Location",
    "Client Lead Location",
    "Work Start Date",
    "OnSite Offshore",
    "Employment Type",
    "Primary Skills",
  ...
  const rows = users.map(user => [
    ...
    escapeCSVValue(user.Work_End_Date),
    escapeCSVValue(user.Client_Partner_Location),
    escapeCSVValue(user.Client_Manager_Location),
    escapeCSVValue(user.Client_Lead_Location),
...
async function handleExport() {
  try {
    const res = await fetch(`http://localhost:8000/users/exportAllemployees`);
